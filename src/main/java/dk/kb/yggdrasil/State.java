 ...
 *      state PRESERVATION_METADATA_DOWNLOAD_SUCCESS
 ...
 *       This can result in failstate PRESERVATION_RESOURCES_DOWNLOAD_FAILURE or the OK-state PRESERVATION_RESOURCES_DOWNLOAD_SUCCESS
 ...
 *  Step 6: Initiate upload to Bitrepository. If initiation fails, go to failstate PRESERVATION_PACKAGE_UPLOAD_FAILURE.
 *  Step 7: Wait for Bitrepository upload to complete. This can result in failstate PRESERVATION_PACKAGE_UPLOAD_FAILURE or
...
public enum State {
    ...
    public static Set<State> getFailStates() {
        Set<State> failstates = new HashSet<State>();
        failstates.add(PRESERVATION_PACKAGE_UPLOAD_FAILURE);
        failstates.add(PRESERVATION_METADATA_DOWNLOAD_FAILURE);
        failstates.add(PRESERVATION_METADATA_PACKAGED_FAILURE);
        failstates.add(PRESERVATION_REQUEST_RECEIVED_BUT_INCOMPLETE);
        failstates.add(PRESERVATION_RESOURCES_DOWNLOAD_FAILURE);
        return failstates;
    }
